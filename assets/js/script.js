(function ($) {
  ...
  /*--------------------- Skills Auto Scroll --------------------*/
  function initSkillsAutoScroll() {
    var skillsRow = document.getElementById("skillsRow");
    var skillsContainer = document.getElementById("skillsContainer");
    
    if (!skillsRow || !skillsContainer) return;
    
    var skillsWrapper = skillsRow.parentElement;
    if (!skillsWrapper) return;
    
    // Clone the skills row for seamless looping
    var clonedRow = skillsRow.cloneNode(true);
    clonedRow.setAttribute("id", "skillsRowClone");
    clonedRow.setAttribute("aria-hidden", "true");
    skillsWrapper.appendChild(clonedRow);
    
    var scrollSpeed = 50; // pixels per second (adjust for speed: lower = slower, higher = faster)
    ...
    function getRowWidth() {
      // Get the actual width including all cards in the 2-row grid
      // Add the margin-right gap (20px) to ensure seamless looping
      var rowWidth = skillsRow.offsetWidth || skillsRow.scrollWidth;
      var marginRight = parseInt(window.getComputedStyle(skillsRow).marginRight) || 20;
      return rowWidth + marginRight;
    }
    ...
    function animateScroll() {
      ...
      var pixelsPerFrame = scrollSpeed * deltaTime;
      
      currentPosition += pixelsPerFrame;
      
      // Reset to 0 when we've scrolled exactly one row width for seamless loop
      if (currentPosition >= rowWidth) {
        currentPosition = currentPosition - rowWidth;
      }
      
      skillsWrapper.style.transform = "translateX(-" + currentPosition + "px)";
      skillsWrapper.style.transition = "none";
  ...
  // Initialize skills auto scroll when document is ready
  function initSkillsOnReady() {
    if (document.readyState === "complete") {
      // Page already loaded
      setTimeout(function() {
        initSkillsAutoScroll();
      }, 300);
    } else {
